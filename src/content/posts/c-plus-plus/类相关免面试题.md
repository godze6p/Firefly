...
**浅拷贝**是指对象复制时，**仅复制指针的值（地址）**，而不复制指针所指向的实际数据。结果是两个对象的指针成员指向**同一块堆内存**。

**深拷贝**是指对象复制时，**为指针成员重新分配内存**，并将原对象指向的数据内容**逐字节复制**到新分配的内存中。结果是两个对象的指针成员指向**各自独立的两块内存**，数据内容相同但互不影响。
...
## **虚函数原理**
**核心机制**：虚函数表（vtable）+ 虚表指针（vptr）

C++ 通过虚函数表（Virtual Table）和虚表指针（Virtual Pointer）实现运行时多态。
